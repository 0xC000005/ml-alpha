...
| B-11 | **Per-month rank-standardize** (paper input contract) | Replace pooled z-score+5σ-clip with per-month **cross-sectional rank** to [−0.5,0.5], missing→cross-sectional median (0); closes a 3-axis fidelity gap with the AIPM input | w33351 §4.1; **only candidate `robust=true`** in enhancement-design workflow (`wf_2f4b6eee`) | small | ✅ **screen PASS (a2rank, EXP-007) → promote** |
...
**First move (revised 2026-06-03 — after EXP-003 + paper + enhancement-design workflow `wf_2f4b6eee`):**
B-00/B-01 are **done-and-weak** (EXP-003: depth non-monotone, width hurts, noise-limited). New ordering:
**B-05 (L1 honest-metric combiner, *precondition*) → B-11 (per-month rank-standardize, the only `robust=true` lever) → one honest-metric MSRR depth screen with a strict monotone kill-bar.** Everything else fell inside the ±0.4–0.5 noise floor. See **Enhancement Roadmap** below.
...
- **Date:** 2026-06-03 · **Status:** 🔬 RUNNING · **Array:** 565895 (24 tasks = base/a1monthz/a2rank × 2012–2019 × **10 seeds**, %2; driver `exp_main_msrr.py`) · **Metric:** L1 SDF Sharpe (raw + L1 logged).
- **Hypothesis:** a2rank's screen win (EXP-007: +0.92 L1 vs base over 3 yrs) holds over the full **8-year** window, where the annualized-Sharpe SE tightens to ~±0.4 (96 months) vs ~±0.5–1.0 per single year. **Bar:** a2rank 8yr L1 **median** beats base by **> the ±0.4 floor**, ideally with a1monthz confirming the 2016-style hard-year robustness. Watch **IC** (flat in the screen — L-07) and per-year dispersion.
- **Next:** collect + interpret (raw vs L1; per-year + 8yr mean/median; IC). Watcher armed.
